...
class hallucinationOutput(BaseModel):
    is_hallucination : bool = Field(description="Whether the LLM output is hallucinated or based on the context documents supplied. True if hallucination, False otherwise")
...
GRADE_DOC_PROMPT='''You are a helpful assistant for grading  of LLM output to check for hallucination based on given LLM output and list of context documents. Given a LLM generated output and Vector store retrieved documents list, your task is to determine whether LLM output is grounded, relevant to the documents. 
